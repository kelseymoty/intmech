...
condition <- "right" #put "right" or "left"

# What do you want the export file to be called?

export_filename <- "Int-Mech2_Success_Right_T2.csv"
...
data <- read.csv("C:/Users/kelse/Desktop//output/Left.csv", header=T)
...
miss_data_cutoff_time1  <- 5437 #cutoff time (in ms) for measuring missing data in trial (just trial 1)
miss_data_cutoff_time2  <- 3437 #cutoff time (in ms) for measuring missing data in trial (trial 2 - 10)
miss_data_cutoff_row1   <- 327 #cutoff time (in rows) for measuring missing data in trial (just trial 1)
miss_data_cutoff_row2   <- 207 #cutoff time (in rows) for measuring missing data in trial (jtrial 2 - 10)
ball_coord_X_left       <- 423 #X coordinate for center of ball (when ball is on left side)
ball_coord_X_right      <- 1497 #X coordinate for center of ball (when ball is on right side)
ball_coord_Y            <- 965 #Y coordinate for center of ball 
...
anticipatory_cutoff1    <- 4120 #looks count as anticipatory before this time (in ms; for trial 1)
anticipatory_cutoff2    <- 2120 #looks count as anticipatory before this time (in ms; for trial 2 - 10)
...
names(TotTrials)[names(TotTrials) == "freq"] <- "totalUseableTrials"
